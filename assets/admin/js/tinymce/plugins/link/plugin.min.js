tinymce.PluginManager.add("link", function (e) {
	...
	function h(t) {
		...
		function o(t) {
			var n = t.meta || {};
			h && h.value(e.convertURL(this.value(), "href")), tinymce.each(t.meta, function (e, t) {
				var n = d.find("#" + t);
		...
		var l, c, u, d, f, p, h, v, b, y, C, x, w = {}, N = e.selection, E = e.dom;
		l = N.getNode(), c = E.getParent(l, "a[href]"), f = a(), w.text = u = c ? c.innerText || c.textContent : N.getContent({format: "text"}), w.href = c ? E.getAttrib(c, "href") : "", c ? w.target = E.getAttrib(c, "target") : e.settings.default_link_target && (w.target = e.settings.default_link_target), (x = E.getAttrib(c, "rel")) && (w.rel = x), (x = E.getAttrib(c, "class")) && (w["class"] = x), (x = E.getAttrib(c, "title")) && (w.title = x), f && (p = {
			name    : "text",
			type    : "textbox",
			size    : 40,
			...
			text : "None",
			value: ""
		}, {text: "New window", value: "_blank"}]), b = {
			name  : "target",
			type  : "listbox",
			label : "Target",
			...
			onSubmit: function (t) {
				...
				function i() {
					...
					e.settings.allow_unsafe_link_target || (t.rel = r(t.rel, "_blank" == t.target)), a === g.href && (g.attach(), g = {}), c ? (e.focus(), f && w.text != u && ("innerText" in c ? c.innerText = w.text : c.textContent = w.text), E.setAttribs(c, t), N.select(c), e.undoManager.add()) : f ? e.insertContent(E.createHTML("a", t, E.encode(w.text))) : e.execCommand("mceInsertLink", !1, t)
				}
	...
	e.addButton("link", {
		...
		tooltip: "Open link",
		onclick: d
	}), e.addContextToolbar(s, "openlink | link unlink")), e.addShortcut("Meta+K", "", p(h)), e.addCommand("mceLink", p(h)), e.on("click", function (e) {
		var t = r(e.target);
		t && tinymce.util.VK.metaKeyPressed(e) && (e.preventDefault(), u(t))
	}), e.on("keydown", function (e) {
		var t = i();
		t && 13 === e.keyCode && v(e) && (e.preventDefault(), u(t))
	}), this.showDialog = h, e.addMenuItem("openlink", {
		text            : "Open link",
		icon            : "newtab",
		onclick         : d,
		onPostRender    : f,
