!function () {
	...
	function t(t, n, r, i, o) {
		...
		function s(t) {
			...
			if (m[t.nodeName] && !p[t.nodeName])return "";
			if (n = "", e(t))return "\n";
			if ((p[t.nodeName] || h[t.nodeName]) && (n += "\n"), t = t.firstChild)do n += s(t); while (t = t.nextSibling);
			return n
		}

		function l(t, n, r) {
			var i, o, a, s, l = [], c = 0, u = t, d = n.shift(), f = 0;
			e:for (; ;) {
				if ((p[u.nodeName] || h[u.nodeName] || e(u)) && c++, 3 === u.nodeType && (!o && u.length + c >= d[1] ? (o = u, s = d[1] - c) : i && l.push(u), !i && u.length + c > d[0] && (i = u, a = d[0] - c), c += u.length), i && o) {
					if (u = r({
							startNode     : i,
							startNodeIndex: a,
							endNode       : o,
				...
				} else if (m[u.nodeName] && !p[u.nodeName] || !u.firstChild) {
					if (u.nextSibling) {
						u = u.nextSibling;
						continue
					}
		...
		function c(e) {
			...
			return function (e) {
				...
				for (var u = t(o.data.substring(e.startNodeIndex), s), d = [], p = 0, m = e.innerNodes.length; p < m; ++p) {
					var h = e.innerNodes[p], g = t(h.data, s);
					h.parentNode.replaceChild(g, h), d.push(g)
				}
		...
		var u, d, f, p, m, h, g = [], v = 0;
		if (f = n.ownerDocument, p = o.getBlockElements(), m = o.getWhiteSpaceElements(), h = o.getShortEndedElements(), d = s(n)) {
			if (t.global)for (; u = t.exec(d);)g.push(a(u, i)); else u = d.match(t), g.push(a(u, i));
	...
	function n(e) {
		function n() {
			...
			var r, i = {};
			r = tinymce.trim(e.selection.getContent({format: "text"}));
			var o = e.windowManager.open({
				...
				items   : {
					...
					labelGap: 30,
					spacing : 10,
					items   : [{type: "textbox", name: "find", size: 40, label: "Find", value: r}, {
						type : "textbox",
						name : "replace",
						size : 40,
						label: "Replace with"
		...
		function i(n) {
			var r, i;
			return i = e.dom.create("span", {"data-mce-bogus": 1}), i.className = "mce-match-marker", r = e.getBody(), u.done(!1), t(n, r, i, !1, e.schema)
		}
		...
		function a(t) {
			var n, i = [];
			if (n = tinymce.toArray(e.getBody().getElementsByTagName("span")), n.length)for (var o = 0; o < n.length; o++) {
				var a = r(n[o]);
		...
		u.init = function (e) {
			...
			var s, f, p, m, h, g, v = d;
			for (n = n !== !1, p = e.getBody(), f = tinymce.grep(tinymce.toArray(p.getElementsByTagName("span")), c), s = 0; s < f.length; s++) {
				var b = r(f[s]);
				if (m = h = parseInt(b, 10), i || m === d) {
					for (t.length ? (f[s].firstChild.nodeValue = t, o(f[s])) : l(f[s]); f[++s];) {
						if (m = parseInt(r(f[s]), 10), m !== h) {
							s--;
							break
						}
						l(f[s])
					...
					n && v--
				} else h > d && f[s].setAttribute("data-mce-index", h - 1)
			}
