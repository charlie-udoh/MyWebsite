tinymce.PluginManager.add("charmap", function (e) {
	...
	function a(t) {
		e.fire("insertCustomChar", {chr: t}).chr, e.execCommand("mceInsertContent", !1, t)
	}
	...
	function s() {
		...
		var l = o(), c = Math.min(l.length, 25), u = Math.ceil(l.length / c);
		for (i = 0; i < u; i++) {
			for (n += "<tr>", r = 0; r < c; r++) {
				var d = i * c + r;
				if (d < l.length) {
					var f = l[d], p = f ? String.fromCharCode(parseInt(f[0], 10)) : "&nbsp;";
					n += '<td title="' + f[1] + '"><div tabindex="-1" title="' + f[1] + '" role="button" data-chr="' + p + '">' + p + "</div></td>"
				} else n += "<td />"
			}
			n += "</tr>"
		}
		n += "</tbody></table>";
		var m = {
			type          : "container", html: n, onclick: function (e) {
				var n = e.target;
